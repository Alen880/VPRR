...
class InputProj(nn.Module):
    def __init__(self, in_channel=3, out_channel=64, kernel_size=3, stride=1, norm_layer=None, act_layer=nn.LeakyReLU):
        ...
        self.proj_s = nn.Sequential(
            nn.Conv2d(1, out_channel, kernel_size=3, stride=stride, padding=1),
            act_layer(inplace=True)
        )
        self.Merge_conv = nn.Conv1d(32, 16, kernel_size=1)
    ...
    def forward(self, x, spare_ref):
        B, C, H, W = x.shape
        x = self.proj(x).flatten(2).transpose(1, 2).contiguous()  # B H*W C
        spare_ref = self.proj_s(spare_ref).flatten(2).transpose(1, 2).contiguous()
        x = torch.cat([x, spare_ref], dim=2)
        x = x.transpose(1, 2)
        x = self.Merge_conv(x)
        x = x.transpose(1, 2)
...
class Uformer(nn.Module):
    def __init__(self, img_size=256, in_chans=3, dd_in=3,
                 ...
                 drop_rate=0.4, attn_drop_rate=0., drop_path_rate=0.1,
        ...
        # self.Merge_conv = nn.Conv2d(in_channels=64, out_channels=32, kernel_size=3, padding=1, stride=1,
        #                             groups=1,
        #                             bias=True)
    ...
    def forward(self, x, spare_ref, mask=None):
        base_x = x
        # Input Projection
        y = self.input_proj(x, spare_ref)
        y = self.pos_drop(y)
        # Encoder
        ...
        return base_x + y if self.dd_in == 3 else y
...
if __name__ == "__main__":
    ...
    model_restoration = Uformer(img_size=input_size, embed_dim=16, depths=depths,
                                ...
                                shift_flag=False)
    result = model_restoration(input, spare)
    print(result.size())
