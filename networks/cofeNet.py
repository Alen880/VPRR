...
class Encode_Decode_seg_2scale_share_atten_res(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6,
                 gpu_ids=[0], use_parallel=True, learn_residual=False, padding_type='reflect'):
        assert (n_blocks >= 0)
...
if __name__ == '__main__':
    ...
    net = Encode_Decode_seg_2scale_share_atten_res(input_nc, output_nc, ngf, norm_layer=norm_layer,
                                                   use_dropout=True, n_blocks=9, gpu_ids=0,
                                                   use_parallel=False, learn_residual=False)
    input256 = torch.randn([1, 3, 256, 256])
    input512 = torch.randn([1, 3, 512, 512])
    print(net(input512, input256, input512).size())
